...
export class ToolHandlers {
  ...
  async handleAuthLogin(browserPath?: string): Promise<{ content: Array<{ type: string; text: string }> }> {
  ...
  async handleAuthStatus(browserPath?: string): Promise<{ content: Array<{ type: string; text: string }> }> {
    const result = await this.authService.checkStatus(browserPath);
    return createToolResponse(result);
  }


  async handleDiscoverFeeds(browserPath?: string): Promise<{ content: Array<{ type: string; text: string }> }> {
    const result = await this.feedService.getFeedList(browserPath);
    return createToolResponse(result);
  }

  async handleSearchNote(keyword?: string, browserPath?: string): Promise<{ content: Array<{ type: string; text: string }> }> {
    validateRequiredParams({ keyword }, ['keyword']);
    const result = await this.feedService.searchFeeds(keyword!, browserPath);
    return createToolResponse(result);
  }

  async handleGetNoteDetail(feedId?: string, xsecToken?: string, browserPath?: string): Promise<{ content: Array<{ type: string; text: string }> }> {
  ...
  async handleCommentOnNote(feedId?: string, xsecToken?: string, note?: string, browserPath?: string): Promise<{ content: Array<{ type: string; text: string }> }> {
  ...
  async handleToolRequest(name: string, args: any): Promise<{ content: Array<{ type: string; text: string }> }> {
    try {
      switch (name) {
        case 'xhs_auth_login':
        ...
        case 'xhs_get_note_detail':
          return await this.handleGetNoteDetail(args?.feed_id as string, args?.xsec_token as string, args?.browser_path as string);

        case 'xhs_comment_on_note':
          return await this.handleCommentOnNote(args?.feed_id as string, args?.xsec_token as string, args?.note as string, args?.browser_path as string);
